...
class LogCreate(BaseModel):
    text: str
    log_type: str = "note"  # note | whatsapp | form
...
async def add_client_log(client_id: str, body: LogCreate):
    log = {"id": new_id(), "text": body.text, "log_type": body.log_type, "created_at": now_iso()}
...
async def seed_defaults():
    ...
    if await db.quick_links.count_documents({}) == 0:
        await db.quick_links.insert_many([
            {"id": new_id(), "title": "موقع AZVIO الرئيسي", "url": "https://azvio.co", "icon": "globe-outline", "created_at": now_iso()},
            {"id": new_id(), "title": "لوحة Netlify", "url": "https://app.netlify.com", "icon": "server-outline", "created_at": now_iso()},
            {"id": new_id(), "title": "لوحة Supabase", "url": "https://supabase.com/dashboard", "icon": "cube-outline", "created_at": now_iso()},
            {"id": new_id(), "title": "Google Drive", "url": "https://drive.google.com", "icon": "folder-open-outline", "created_at": now_iso()},
        ])
