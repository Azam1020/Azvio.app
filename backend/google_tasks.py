...
async def _get_selected_tasklist_ids(user_id: str, account_email: str) -> list:
    doc = await db.google_accounts.find_one(
        {"user_id": user_id, "email": account_email},
        {"_id": 0, "selected_tasklist_ids": 1, "selected_tasklist_id": 1},
    )
    doc = doc or {}
    ids = doc.get("selected_tasklist_ids")
    if ids:
        return ids
    old_single = doc.get("selected_tasklist_id")  # توافق مع النظام القديم (اختيار وحد)
    return [old_single] if old_single else ["@default"]
...
async def list_tasklists(account: str = Query(...), current_user=Depends(get_current_user)):
    """كل قوائم المهام بحساب Google — يختار المستخدم منها (يدعم أكثر من قائمة بنفس الوقت)."""
    ...
    selected = await _get_selected_tasklist_ids(current_user["user_id"], account)
    return {"lists": lists, "selected_tasklist_ids": selected}


@router.post("/gtasks/select", dependencies=[Depends(get_current_user)])
async def select_tasklist(
    account: str = Query(...),
    tasklist_ids: str = Query(..., description="قائمة معرّفات قوائم مهام مفصولة بفاصلة"),
    current_user=Depends(get_current_user),
):
    ids_list = [t.strip() for t in tasklist_ids.split(",") if t.strip()]
    if not ids_list:
        raise HTTPException(status_code=400, detail="لازم تختار قائمة مهام واحدة على الأقل")
    result = await db.google_accounts.update_one(
        {"user_id": current_user["user_id"], "email": account},
        {"$set": {"selected_tasklist_ids": ids_list}},
    )
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="الحساب غير موجود")
    return {"ok": True, "selected_tasklist_ids": ids_list}
...
async def list_gtasks(account: str = Query(...), current_user=Depends(get_current_user)):
    access_token, _ = await _get_valid_credentials(current_user["user_id"], account)
    tasklist_ids = await _get_selected_tasklist_ids(current_user["user_id"], account)
    all_items = []
    for tl_id in tasklist_ids:
        try:
            items = await run_in_threadpool(_sync_list_tasks, access_token, tl_id)
            for it in items:
                it["_azvio_tasklist_id"] = tl_id
            all_items.extend(items)
        except Exception:
            continue
    return {"tasks": all_items, "tasklist_ids": tasklist_ids}
...
async def create_gtask(body: GTaskCreate, current_user=Depends(get_current_user)):
    ...
    tasklist_ids = await _get_selected_tasklist_ids(current_user["user_id"], body.account_email)
    tasklist_id = body.tasklist_id or tasklist_ids[0]  # افتراضياً أول قائمة مختارة، أو محددة صراحة
...
async def update_gtask(body: GTaskUpdate, current_user=Depends(get_current_user)):
    access_token, _ = await _get_valid_credentials(current_user["user_id"], body.account_email)
    tasklist_ids = await _get_selected_tasklist_ids(current_user["user_id"], body.account_email)
    tasklist_id = body.tasklist_id or tasklist_ids[0]
    patch: dict = {}
...
async def delete_gtask(
    ...
    access_token, _ = await _get_valid_credentials(current_user["user_id"], account)
    if tasklist_id:
        real_tasklist_id = tasklist_id
    else:
        tasklist_ids = await _get_selected_tasklist_ids(current_user["user_id"], account)
        real_tasklist_id = tasklist_ids[0]
    try:
...
async def sync_from_google(account: str = Query(...), current_user=Depends(get_current_user)):
    """يسحب كل المهام من كل قوائم Google Tasks المختارة ويحدّث/ينشئ نسخها المحلية — يُستخدم عند
    فتح شاشة اليوم أو بالسحب للتحديث، عشان أي تعديل أو حذف صار مباشرة بتطبيق Google Tasks ينعكس بالتطبيق."""
    access_token, _ = await _get_valid_credentials(current_user["user_id"], account)
    tasklist_ids = await _get_selected_tasklist_ids(current_user["user_id"], account)
    ...
    # المهام اللي انحذفت من قوقل تنحذف محلياً برضو — بس بس ضمن القوائم المختارة حالياً
    await db.tasks.delete_many({
        "google_tasklist_id": {"$in": tasklist_ids},
